...
if (isset($_POST['tambah-dpp'])) {
    $tahun = $_POST['tahun'];
    $nominal = $_POST['nominal'];

    $cektahun = mysqli_query($koneksi, "SELECT * FROM dpp WHERE tahun = '$tahun'");

    if (mysqli_num_rows($cektahun) > 0) {
        $_SESSION['icon'] = "error";
        $_SESSION['title'] = "Gagal";
        $_SESSION['text'] = "DPP Tahun Ini Sudah Ada !";
        header('location:index.php?page=dpp');
    } else {
        $query = mysqli_query($koneksi, "INSERT INTO dpp (tahun,nominal) VALUES('$tahun','$nominal')");
...
if (isset($_POST['edit-dpp'])) {
    $id = $_POST['id_dpp'];
    $tahun = $_POST['tahun'];
    $nominal = $_POST['nominal'];

    $cektahun = mysqli_query($koneksi, "SELECT * FROM dpp WHERE tahun = '$tahun' AND id_dpp!='$id'");
    ...
    if (mysqli_num_rows($cektahun) > 0) {
        ...
        $query = mysqli_query($koneksi, "UPDATE dpp SET tahun='$tahun',nominal='$nominal' WHERE id_dpp='$id'");
...
if (!empty($_GET['submit'] == 'hapusdpp')) {
    $id = $_GET['id_dpp'];
    $query = mysqli_query($koneksi, "DELETE FROM dpp WHERE id_dpp='$id'");
...
if (isset($_POST['tambah-pembayaran'])) {
    ...
    $cektransaksi = mysqli_query($koneksi, "SELECT * FROM pembayaran WHERE nisn = '$nisn' AND id_dpp='$id_dpp'");
    $dpp = mysqli_query($koneksi, "SELECT * FROM dpp WHERE id_dpp='$id_dpp'");
    $data = mysqli_fetch_array($dpp);

    if (mysqli_num_rows($cektransaksi) > 0) {
        $_SESSION['icon'] = "error";
        $_SESSION['title'] = "Gagal";
        $_SESSION['text'] = "Siswa Telah Melakukan Transaksi Untuk DPP Ini !";
        header('location:index.php?page=laporan');
    } else {
        if ($jumlah_bayar > $data['nominal']) {
            $_SESSION['icon'] = "error";
            ...
            $query = mysqli_query($koneksi, "INSERT INTO pembayaran (id_petugas, nisn, tgl_bayar, id_dpp, jumlah_bayar) VALUES ('$id_petugas', '$nisn', '$tanggal_bayar', '$id_dpp', '$jumlah_bayar')");
            if ($query) {
                $_SESSION['icon'] = "success";
                $_SESSION['title'] = "Berhasil";
                $_SESSION['text'] = "DPP Telah DiBayar !";
                header('location:index.php?page=laporan');
            }
        }
    }
